...
function fromX64(num: BN) {
    return new Decimal(num.toString()).mul(Decimal.pow(2, -64));
}

function tickIndexToSqrtPriceX64(tickIndex: number) {
    if (tickIndex > 0) {
...
function tickIndexToPrice(tickIndex: number, decimalsA: number, decimalsB: number) {
...
export function whirlpoolTokenBalances(
    liquidity: BN,
    sqrtPrice: BN,
    ...
    const depositA = liquidity.toNumber() * (1/clamped_current_sqrt_price - 1/upper_sqrt_price) / (10 ** (-1 * sqrtDecimalGap));
    const depositB = liquidity.toNumber() * (clamped_current_sqrt_price - lower_sqrt_price) / (10 ** (sqrtDecimalGap));
